...
import { hashPassword, comparePassword, generateVerificationCode, isValidPhone, isValidEmail } from "../utils/auth";
import { sendEmail, sendSMS, getVerificationCodeTemplate, getSMSTemplate } from "../utils/notification";

const JWT_SECRET = process.env.JWT_SECRET || "your-secret-key";

// --- Business Logic Functions ---

export async function registerUser(userData: UserRegistration & { phone_number: string }): Promise<{ id: number; token: string }> {
  const { username, email, password, phone_number, full_name, avatar_url } = userData;
  ...
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO users (username, email, password, phone_number, full_name, avatar_url, role, phone_verified, email_verified) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [username, email, hashedPassword, phone_number, full_name || null, avatar_url || null, "client", false, false],
    ) as any;
...
export async function loginUser(credentials: UserCredentials): Promise<{ user: Partial<User>; token: string }> {
...
export async function sendVerificationCode(recipient: string, type: 'phone' | 'email', purpose: 'login' | 'register'): Promise<void> {
  const connection = await getConnection();
  try {
    // 检查发送频率（1分钟内只能发送一次）
...
export async function sendVerificationCodeAuto(recipient: string, purpose: 'login' | 'register'): Promise<void> {
  // 自动识别输入类型
...
export async function loginWithCodeAuto(recipient: string, code: string): Promise<{ user: Partial<User>; token: string }> {
  // 自动识别输入类型
  let type: 'phone' | 'email';
  if (isValidPhone(recipient)) {
    type = 'phone';
...
export async function loginWithCode(recipient: string, code: string, type: 'phone' | 'email'): Promise<{ user: Partial<User>; token: string }> {
  const connection = await getConnection();
  try {
    // 验证验证码
...
export async function getUsers(options: {
  ...
  const connection = await getConnection();
  try {
    let sql = "SELECT id, username, email, phone_number, full_name, role, created_at, status, avatar_url, phone_verified, email_verified FROM users";
    const values = [];
